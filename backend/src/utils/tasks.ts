...
export function bumpTaskProgress(userId: string, taskId: string): void {
...
export function getDailyTasks(userId: string): DailyTaskState[] {
...
export function claimTask(userId: string, taskId: string): number {
  const def = DEF_MAP.get(taskId);
  ...
  db.prepare('UPDATE users SET coins = coins + ? WHERE id = ?').run(def.reward, userId);
  return def.reward;
}
